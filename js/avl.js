...
class AVLTree {
    ...
    processOperations() {
        const buttons = [document.getElementById('insertBtn'), document.getElementById('deleteBtn'), document.getElementById('searchBtn')];
    ...
    animateNode(node) {
        if (!node) return;
        if (node.arcPath) this.animateArc(node);
        else {
            node.x += (node.targetX - node.x) * this.animationSpeed;
            node.y += (node.targetY - node.y) * this.animationSpeed;
        }
        if (node.left) this.animateNode(node.left);
        if (node.right) this.animateNode(node.right);
    }
    ...
    animate() {
        ...
        if (this.root) this.animateNode(this.root);
